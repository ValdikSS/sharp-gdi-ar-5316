...
{
    bool inRun = false;
    uint32_t destCount = 0;             // Destination buffer used
    uint32_t bytesPending = 0;          // Bytes looked at but not yet output
    const uint8_t *pendingPtr;          // Pointer to first pending byte
    uint32_t runStart = 0;              // Distance into pending bytes that a run starts
...
uint32_t unpackbits(const uint8_t *srcPtr, uint8_t *destPtr, uint32_t srcCount, uint32_t destLimit)
{
    ...
    uint32_t srcRemaining;          // Number of bytes of source left to unpack
    uint32_t destRemaining;         // Buffer size still available for unpacking into
